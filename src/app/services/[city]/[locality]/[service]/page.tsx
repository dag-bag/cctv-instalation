...
async function getPageData(city: string, locality: string, service: string) {
  ...
  const isElectrical =
    serviceLower.includes("electrical") ||
    serviceLower.includes("wiring") ||
    serviceLower.includes("power") ||
    serviceLower.includes("mcb") ||
    serviceLower.includes("earthing");

  const pricing = isElectrical
    ? [
        { item: "Wiring (per meter)", price: "Get Quote", cta: "Call Us" },
        {
          item: "Switch/Socket Installation",
          price: "Get Quote",
          cta: "Call Us",
        },
    ...
    <div className={styles.container}>
      ...
      <nav className={styles.breadcrumbs} aria-label="Breadcrumb">
        <div className={styles.breadcrumbContent}>
          ...
          &gt;{" "}
          <Link href={`/services/${citySlug}`} className={styles.link}>
            {city}
          </Link>{" "}
            ...
            <div className={styles.pricingTable}>
              {data.pricing.map((item, index) => (
                <div key={index} className={styles.pricingRow}>
                  ...
                  <button className={styles.pricingButton}>{item.cta}</button>
                </div>
              ))}
            </div>
          ...
          <div className={styles.relatedServices}>
            ...
            <ul className={styles.relatedList}>
              {SERVICES.slice(0, 5).map((s, i) => {
                ...
                return (
                  <li key={i}>
                    <Link href={slug} className={styles.relatedLink}>
                      {s}
                    </Link>
                  </li>
                );
      ...
      <footer className={styles.footer}>
        <h2 className={styles.footerTitle}>
          Ready to secure your property in {locality}?
        </h2>
        <div className={styles.footerButtons}>
          <button className={styles.ctaButton}>
            Call Now: +91 87662 03976
          </button>
          <button className={styles.outlineButton}>WhatsApp Us</button>
        </div>
      </footer>
