...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description,
    keywords: [
      `CCTV installation ${locality}`,
      `CCTV repair ${locality}`,
      `security systems ${locality}`,
      `camera installation ${city}`,
      `best cctv camera ${locality}`,
      `cctv camera price ${locality}`,
    ],
    openGraph: {
      title,
      description,
...
export default async function LocalityPage({ params }: Props) {
  ...
  return (
    <div className={styles.container}>
      ...
      <header className={styles.hero}>
        ...
        <div className={styles.heroContent}>
          <h1 className={styles.title}>
            Services in <span className={styles.highlight}>{locality}</span>,{" "}
            {city}
          </h1>
      ...
      <nav className={styles.breadcrumbs} aria-label="Breadcrumb">
        <div className={styles.breadcrumbContent}>
          ...
          <Link
            ...
            aria-label={`Explore services in ${city}`}
          >
            {city}
          </Link>{" "}
        ...
        <div className={styles.contentSection} style={{ width: "100%" }}>
          <section>
            <h2 className={styles.sectionTitle}>
              Available Services in {locality}, {city}
            </h2>
          ...
          <section style={{ marginTop: "4rem" }}>
            <h2 className={styles.sectionTitle}>
              Security Services in {locality}, {city}
            </h2>
            <div className={styles.featuresGrid}>
              <div className={styles.featureCard}>
                <span className={styles.checkIcon}>⚡</span>
                <span className={styles.featureText}>
                  Quick Response in {locality}
                </span>
              </div>
              <div className={styles.featureCard}>
                <span className={styles.checkIcon}>👨‍🔧</span>
            ...
            <div className={styles.text} style={{ marginTop: "2rem" }}>
              <p>
                Residents and businesses in <strong>{locality}</strong> rely on
                CamHarbor for top-tier security installations. From residential
                CCTV setups to complex commercial access control systems, we
                provide tailored solutions that meet your specific needs. Our
                local team in {city} ensures prompt service and ongoing support
                for all our clients in {locality}.
              </p>
            </div>
          </section>
        </div>
      ...
      <section style={{ marginTop: "4rem" }}>
        <h2 className={styles.sectionTitle}>
          Other Areas near {locality} in {city}
        </h2>
        <div className={styles.featuresGrid}>
          {(LOCALITIES[city] || [])
            .filter((l) => l !== locality)
